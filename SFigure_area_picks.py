...
# %%
pks,_ = GPA.extract_primary_ks(meancrop2, pix_norm_range=(15,100), plot=True, threshold=0.001, sigma=0.8)

# %%
all_ks = np.concatenate([pks, [pks[0]+pks[1]]])
for pk in pks2:
    plt.scatter(*np.stack([pk, -pk]).T/biNMPERPIXEL, marker='o')
...
# %%
from latticegen import combine_ks
all_ks = combine_ks(np.concatenate([pks, -pks])/biNMPERPIXEL, order=2)
#all_ks = all_ks[np.abs(all_ks[:,1]) < 0.12,:]
plt.scatter(*all_ks.T[::-1], marker='o')
plt.gca().set_aspect('equal')

# %%
biNMPERPIXEL/np.linalg.norm(all_ks, axis=1).max()/(0.5*np.sqrt(3))

# %%
plt.imshow(fftim.T, extent=[-0.5,0.5,-0.5,0.5], vmax=1e7)
plt.gca().set_xlim([-rf,rf])
plt.gca().set_ylim([-rf,rf])
plt.colorbar()
...
# %%
fftimhan = np.abs(np.fft.fftshift(np.fft.fft2(window * (meancrop2-meancrop2.mean()))))
plt.imshow(fftimhan.T, extent=[-0.5,0.5,-0.5,0.5], vmax=0.1e7)
plt.gca().set_xlim([-rf,rf])
plt.gca().set_ylim([-rf,rf])
plt.colorbar()
